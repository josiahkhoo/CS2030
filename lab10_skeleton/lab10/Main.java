...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    List<CompletableFuture<BusRoutes>> futuresBusRoutes = new ArrayList<>();
    try {
      Scanner sc = createScanner(args);
      while (sc.hasNext()) {
        BusStop srcId = new BusStop(sc.next());
        String searchString = sc.next();
        CompletableFuture<BusRoutes> future = BusSg.findBusServicesBetweenAsync(srcId, searchString);
        futuresBusRoutes.add(future);
      }
      sc.close();
    } catch (FileNotFoundException exception) {
      System.err.println("Unable to open file " + args[0] + " " + exception);
    }
    List<CompletableFuture<String>> futures = futuresBusRoutes.stream()
        .map(future -> future.thenApply(busRoutes -> busRoutes.description())).collect(Collectors.toList());
