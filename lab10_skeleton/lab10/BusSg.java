...
class BusSg {
  ...
  public static CompletableFuture<BusRoutes> findBusServicesBetween(BusStop stop, String searchString) {
    try {
      CompletableFuture<Set<BusStop>> busStops = stop.getBusServices().thenApply(x -> x.stream()
      ...
      return CompletableFuture.supplyAsync(() -> BusRoutes(stop, searchString, Map.of()));
    }
  }
