...
class Lazy<T extends Comparable<T>> {
    ...
    public <R extends Comparable<R>> Lazy<R> map(Function<T,R> function) {
        Supplier<R> mapSupplier = (() -> function.apply(this.get()));
        return new Lazy<R>(mapSupplier);
    }

    public <R extends Comparable<R>> Lazy<R> flatMap(Function<T, Lazy<R>> function) {
        Supplier<R> flatMapSupplier = (() -> function.apply(this.get()).get());
        return new Lazy<R>(flatMapSupplier);
    }

    public <U extends Comparable<U>,R extends Comparable<R>> Lazy<R> combine(Lazy<U> otherLazy, BiFunction<T,U,R> function) {
    ...
    public Lazy<Integer> compareTo(Lazy<T> otherLazy) {
        Supplier<Integer> compareToSupplier = (() -> this.get().compareTo(otherLazy.get()));
        return new Lazy<Integer>(compareToSupplier);
    }
